...
def filter_row(row, threshold=0.98):
    # 获取原始索引
...
if __name__== "__main__":
    ...
    # adj_layer1 = adj_layer1.max() * np.eye(adj_layer1.shape[0]) + adj_layer1
    # adj_layer2 = adj_layer2.max() * np.eye(adj_layer2.shape[0]) + adj_layer2
    # adj_layer3 = adj_layer3.max() * np.eye(adj_layer3.shape[0]) + adj_layer3

    #adj_layer4 = adj_layer4.max() * np.eye(adj_layer4.shape[0]) + adj_layer4

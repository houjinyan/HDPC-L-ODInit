...
operator1=layer1_step/layer4_step
operator2=layer2_step/layer4_step
operator3=layer3_step/layer4_step
...
if __name__== "__main__":
    ...
    file_name_origin="../dens_shenzhen/dens_shenzhen_test_"+str(layer4_step)+".csv"
    ...
    for i in range(0,layer1_clusters_num):
        for j in range(0,layer2_clusters_num):
            for k in range(0,layer3_clusters_num):
                with open("../cluster_layers/" + "layer" + str(layer) + "/layer" + str(layer) + "_(layer1-cluster" + str(i) + "-layer2-cluster"+str(j)+"-layer3-cluster"+str(str(k))+"-dens-" + str(layer4_step) + ").csv", "w") as fin:
                    fin.write("lng,lat,dens,row,col,layer1-cluster,layer2-cluster,layer3-cluster,layer\n")
    ...
    for i in range(0,origin_datas.shape[0]):
        ...
        layer3_datas=pd.read_csv('../cluster_layers/layer3/layer3_clusters_(layer1-cluster'+str(layer1_cluster_order)+'-layer2-cluster'+str(layer2_cluster_order)+'-dens-'+str(layer3_step)+').csv')
        try:
            layer3_cluster_order=layer3_datas[(layer3_datas['row'] == floor(operator3 * origin_datas['row'][i])) & (layer3_datas['col'] == floor(operator3 * origin_datas['col'][i]))]['layer3-cluster'].tolist()[0]
        except:
            print('../cluster_layers/layer3/layer3_clusters_(layer1-cluster'+str(layer1_cluster_order)+'-layer2-cluster'+str(layer2_cluster_order)+'-dens-'+str(layer3_step)+').csv')
        with open("../cluster_layers/" + "layer" + str(layer) + "/layer" + str(layer) + "_(layer1-cluster" + str(layer1_cluster_order) + "-layer2-cluster"+str(layer2_cluster_order)+"-layer3-cluster"+str(str(layer3_cluster_order))+"-dens-" + str(layer4_step) + ").csv", "a") as fin:
            ...
            fin.write(',')
            fin.write(str(layer3_cluster_order))
            fin.write(',')
            fin.write(str(layer))
            fin.write('\n')

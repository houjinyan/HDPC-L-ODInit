...
def get_dataloader(data_dir, dataset, batch_size, test_batch_size, io,isweighted,scalar_type='Standard'):
    data = {}
    for category in ['train', 'val', 'test']:
        if(isweighted=='weighted'):
            cat_data = np.load(os.path.join(data_dir, dataset,io+'_'+isweighted, category + '.npz'))
        else:
            cat_data = np.load(os.path.join(data_dir, dataset, io , category + '.npz'))

...
def aug_topology(sim_mx, input_graph, percent=0.2):
    ...
    edge_num=drop_prob.shape[0]
    drop_prob = (1. - drop_prob).numpy() # normalized similarity to get sampling probability
    drop_prob /= drop_prob.sum()
    # # 找到最小的N个数的索引
    # indices_to_remove = np.argpartition(drop_prob, drop_prob.shape[0]-edge_num)[:drop_prob.shape[0]-edge_num]
    # # 创建新的数组，不包括最小的N个数
    # drop_prob = np.delete(drop_prob, indices_to_remove)

...
import numpy as np
import torch
from lib.metrics import r2,explained_variance,accuracy,rmse
...
class Trainer(object):
    def __init__(self, model, optimizer, dataloader, graph, args,start_time):
    ...
    def train_epoch(self, epoch, loss_weights):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            loss, sep_loss,y_pred,y_true = self.model.loss(repr1, repr2, target, self.scaler, loss_weights)
        ...
        y_true_epoch = torch.tensor(np.concatenate(y_true_list, axis=0))
        y_pred_epoch = torch.tensor(np.concatenate(y_pred_list, axis=0))

        rmse_print = rmse(y_true_epoch,y_pred_epoch).item()
        acc_print = accuracy(y_true_epoch, y_pred_epoch).item()
        r_2_print = r2(y_true_epoch, y_pred_epoch).item()
        e_v_print = explained_variance(y_true_epoch, y_pred_epoch).item()
        self.logger.info('*******Train Epoch {}: averaged Loss : {:.6f}'.format(epoch, train_epoch_loss))
        print("epoch: ",epoch,"  accuracy: ",acc_print)
        with open(self.args.dataset+"_"+self.args.IO+"_"+self.args.isweighted+'_train.csv','a') as f:
            f.write(str(epoch))
            f.write(',')
            f.write(str(acc_print))
            f.write(',')
            f.write(str(rmse_print))
            f.write(',')
            f.write(str(r_2_print))
            f.write(',')
            f.write(str(e_v_print))
            f.write(',')
            f.write(str(time.perf_counter() - self.start_time))
            f.write('\n')
    ...
    def val_epoch(self, epoch, val_dataloader, loss_weights):
        ...
        with open(self.args.dataset+"_"+self.args.IO+"_"+self.args.isweighted+'_val.csv','a') as f:
            f.write(str(epoch))
            f.write(',')
            f.write(str(acc_print))
            f.write(',')

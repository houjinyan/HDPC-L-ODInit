...
def train(model, data_loader, optimizer, loss_function, epoch, metric, config):
    ...
    with open('HKIn_noweighted_train.csv','a') as f:
        f.write(str(epoch))
        f.write(',')
...
def evals(model, data_loader, epoch, metric, config, mode='Test', end=''):
    with torch.no_grad():
        ...
        time_cost = np.sum(time_lst)
        metric.update_metrics(y_true,y_pred)
        with open('HKIn_noweighted_val.csv', 'a') as f:
            f.write(str(epoch))
            f.write(',')
            f.write(str(metric.metrics['acc']))
            f.write(',')
...
def main(config):
    ...
    with open('HKIn_noweighted_train.csv','w') as f:
        f.write('epoch,accuracy,rmse,R2,E-V\n')
    with open('HKIn_noweighted_val.csv','w') as f:
        f.write('epoch,accuracy,rmse,R2,E-V\n')
...
def default_config(data='PEMS08',workname='record'):
    ...
    # config.data.feature_file = config.data.path+config.data.name+'/in_weighted/layer4_dataset_s.csv'
    # config.data.spatial = config.data.path+config.data.name+'/in_weighted/adj_layer4_weighted.csv'
    config.data.feature_file = config.data.path+config.data.name+'/in/layer4_dataset_s.csv'
    config.data.spatial = config.data.path+config.data.name+'/in/adj_layer4.csv'

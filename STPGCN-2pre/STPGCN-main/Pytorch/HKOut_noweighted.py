...
def accuracy(y_true,y_pred):
    return 1 - np.linalg.norm(y_true - y_pred) / np.linalg.norm(y_true)
...
class Metric(object):
    ...
    def __init__(self, num_prediction):
        ...
        self.best_metrics  = {'mae':np.inf, 'rmse':np.inf, 'mape':np.inf, 'epoch': np.inf,'r2':np.inf,'e_v':np.inf,'acc':np.inf}
        self.step_metrics_epoch = {'mae':{}, 'rmse':{}, 'mape':{},'r2':{},'e_v':{},'acc':{}}

    def update_metrics(self, y_true, y_pred):
        self.metrics = {'mae':0.0, 'rmse':0.0, 'mape':0.0, 'time':0.0, 'r2':0.0, 'e_v':0.0,'acc':0.0}
        self.metrics['mae'], self.metrics['rmse'], self.metrics['mape'],  self.metrics['r2'],self.metrics['e_v'],self.metrics['acc']= self.get_metric(y_true, y_pred)
        self.metrics['time'] = time_to_str((timer() - self.time_start))

    def update_step_metrics(self, y_true, y_pred, epoch=0):
        idx_lst=['mae','rmse','mape','r2','e_v','acc']
        
        metrics = {}
        for i in idx_lst:
            metrics[i] = [0.0]*(self.num_prediction+1)

        metrics['mae'][-1], metrics['rmse'][-1], metrics['mape'][-1], metrics['r2'][-1],metrics['e_v'][-1],metrics['acc'][-1]= self.get_metric(y_true, y_pred)

        for t in range(self.num_prediction):
            true, pred = y_true[:,t,:,:], y_pred[:,t,:,:]
            metrics['mae'][t], metrics['rmse'][t], metrics['mape'][t], metrics['r2'][t],metrics['e_v'][t],metrics['acc'][t] = self.get_metric(true, pred)
    ...
    def update_best_metrics(self, epoch=0):
        ...
        self.best_metrics['r2'], r_2_state = self.get_best_metric(self.best_metrics['r2'], self.metrics['r2'])
        self.best_metrics['e_v'], ev_state = self.get_best_metric(self.best_metrics['e_v'], self.metrics['e_v'])
        self.best_metrics['acc'], acc_state = self.get_best_metric(self.best_metrics['acc'], self.metrics['acc'])
    ...
    def get_metric(y_true, y_pred):
        ...
        e_v = explained_variance(y_true, y_pred)
        acc=accuracy(y_true, y_pred)
        return mae, rmse, mape, r_2,e_v,acc
    ...
    def __str__(self):
        """For print"""
        return f"{self.metrics['rmse']:<7.6f}{' '}{self.metrics['r2']:<7.6f}{' '}{self.metrics['e_v']:<7.6f}{' '}{self.metrics['acc']:<7.6f} | {self.best_metrics['epoch']+1:<4}"

    def best_str(self):
        """For save"""
        return f"{self.best_metrics['epoch']}{'| '}{self.best_metrics['rmse']:.6f}{' '}{self.best_metrics['r2']:.6f}{' '}{self.best_metrics['e_v']:.6f}{' '}{self.best_metrics['acc']:.6f}"

    def multi_step_str(self, obj='rmse', sep=',', epoch=0):
        """For print or save""" #"{i+1}:{x:<7.2f}"
        return sep.join([f"{x:.6f}" if sep ==',' else f"{x:<6.6f}" for i,x in enumerate(self.step_metrics_epoch[obj][epoch])])
...
class CleanDataset():
    def __init__(self, config):
        ...
        self.adj =np.array(pd.read_csv(config.data.spatial, header=0, index_col=0))
    ...
    def read_data(self):
        if 'PEMS' in self.data_name:
            ...
            data=np.array(pd.read_csv(self.feature_file, header=0,index_col=0))
...
def train(model, data_loader, optimizer, loss_function, epoch, metric, config):
    ...
    for i, batch in enumerate(data_loader):
        ...
        message = f"{i/len(data_loader)+epoch:6.6f} Time:{np.sum(time_lst):.6f}s"
    ...
    print("epoch: ",epoch,' rmse: ',metric.metrics['rmse'],' r2: ',metric.metrics['r2'],' e_v: ',metric.metrics['e_v'],' acc: ',metric.metrics['acc'])
    with open('HKOut_noweighted_train.csv','a') as f:
        f.write(str(epoch))
        f.write(',')
        f.write(str(metric.metrics['acc']))
        f.write(',')
        f.write(str(metric.metrics['rmse']))
        f.write(',')
        f.write(str(metric.metrics['r2']))
        f.write(',')
        f.write(str(metric.metrics['e_v']))
        f.write(',')
        f.write(str(time.perf_counter()-time_start_self))
        f.write('\n')
    #message = f"{epoch+1:<3} | {metric.metrics['mae']:<7.6f}{metric.metrics['rmse']:<7.6f}{metric.metrics['mape']:<7.6f}{time_cost:<5.6f}s"
    #print('\r'+message , end='', flush=False)
    
    # message = f"{'Train':5}{epoch+1:6.6f} | {str(metric)}{time_cost:.6f}s"
    # config.logger.write('\n'+message+'\n',is_terminal=False)
...
def evals(model, data_loader, epoch, metric, config, mode='Test', end=''):
    with torch.no_grad():
        ...
        time_cost = np.sum(time_lst)
        metric.update_metrics(y_true,y_pred)
        with open('HKOut_noweighted_val.csv', 'a') as f:
            f.write(str(epoch))
            f.write(',')
            f.write(str(metric.metrics['acc']))
            f.write(',')
        ...
        #message = f" | {metric.metrics['mae']:<7.6f}{metric.metrics['rmse']:<7.6f}{metric.metrics['mape']:<7.6f}{time_cost:<5.6f}s"
        #print(message , end=end, flush=False)
        # message = f"{mode:5}{epoch+1:6.6f} | {str(metric)}{time_cost:.6f}s"
...
def main(config):
    ...
    with open('HKOut_noweighted_train.csv','w') as f:
        f.write('epoch,accuracy,rmse,R2,E-V\n')
    with open('HKOut_noweighted_val.csv','w') as f:
        f.write('epoch,accuracy,rmse,R2,E-V\n')
    ...
    # config.model.logger.write(f"Best Validation MAE at Epoch:{best_val_epoch+1:^3} | Corresponding Testing MAE:{metrics_test.step_metrics_epoch['mae'][best_val_epoch][-1]:.6f} RMSE:{metrics_test.step_metrics_epoch['rmse'][best_val_epoch][-1]:.6f} MAPE:{metrics_test.step_metrics_epoch['mape'][best_val_epoch][-1]:.6f}\n",is_terminal=True)
...
def default_config(data='PEMS08',workname='record'):
    ...
    # config.data.feature_file = config.data.path+config.data.name+'/out_weighted/layer4_dataset_s.csv'
    # config.data.spatial = config.data.path+config.data.name+'/out_weighted/adj_layer4_weighted.csv'
    config.data.feature_file = config.data.path+config.data.name+'/out/layer4_dataset_s.csv'
    config.data.spatial = config.data.path+config.data.name+'/out/adj_layer4.csv'
    ...
    if config.data.name == 'Shenzhen':
        config.data.num_features    = 1
        config.data.num_vertices    = 1120
        config.data.points_per_hour = 4
        config.data.val_start_idx   = int(4032 * 0.6)
        config.data.test_start_idx  = int(4032 * 0.8)

    if config.data.name == 'Haikou':
        config.data.num_features    = 1
        config.data.num_vertices    = 180
        config.data.points_per_hour = 2
        config.data.val_start_idx   = int(8832 * 0.6)
        config.data.test_start_idx  = int(8832 * 0.8)
...
if __name__ == "__main__":
    time_start_self = time.perf_counter()

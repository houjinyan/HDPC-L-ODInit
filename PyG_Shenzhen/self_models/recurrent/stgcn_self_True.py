...
import torch
from torch_geometric_temporal.nn.recurrent import STGCN
from torch_geometric_temporal.dataset import ShenzhenDatasetLoader_layer4_weighted,ShenzhenDatasetLoader_layer4_noweighted,ShenzhenDatasetLoader_layer3_weighted,ShenzhenDatasetLoader_layer3_noweighted
...
layer4_adj_weighted_filename='data/out_weighted/adj_layer4_weighted.csv'
if(WEIGHTED==True):
...
adj_weighted_layer4=torch.torch.from_numpy(adj_weighted_layer4).type(torch.FloatTensor).to(DEVICE)

if(WEIGHTED==True):
    loader = ShenzhenDatasetLoader_layer4_weighted()
else:
    loader = ShenzhenDatasetLoader_layer4_noweighted()
...
class TemporalGNN(torch.nn.Module):
    def __init__(self, num_nodes, num_features, time_len,pre_len):
        super(TemporalGNN, self).__init__()
        # Attention Temporal Graph Convolutional Cell
        self.tgnn = STGCN(num_nodes, num_features, time_len,pre_len)
    ...
    def forward(self, adj, x):
        ...
        """
        h = self.tgnn(adj,x)  # x [b, 207, 2, 12]  returns h [b, 207, 12]
        #h = self.sig(h)
        #h = self.linear(h)
        return h

# Create model and optimizers
model = TemporalGNN(adj_weighted_layer4.shape[0], 1, TIME_LEN, PRE_LEN).to(DEVICE)
...
for epoch in range(EPOCH):
    ...
    for encoder_inputs, labels in train_loader:
        ...
        y_hat = model(adj_weighted_layer4,encoder_inputs)  # Get model predictions
    ...
    print("Epoch {} train Accuracy: {:.6f}".format(epoch, sum(accuracy_list) / len(accuracy_list)))
    ...
    with torch.no_grad():
        ...
        for encoder_inputs, labels in test_loader:
            ...
            y_hat = model(adj_weighted_layer4, encoder_inputs)
            # Mean squared error
    ...
    print('test-accuracy: ',(sum(accuracy_list) / len(accuracy_list)).item())
...
df.to_csv('STGCN_Train_Log_'+str(EPOCH)+'epoch_' + str(WEIGHTED) + '.csv')
df_test.to_csv('STGCN_Test_Log_'+str(EPOCH)+'epoch_' + str(WEIGHTED) + '.csv')
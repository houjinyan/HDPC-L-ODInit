...
class TemporalGNN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        """
        h = self.tgnn(x)  # x [b, 207, 2, 12]  returns h [b, 207, 12]
        h = self.sig(h)
        h = self.linear(h)
        return h
...
df.to_csv('TGCN_Train_Log_'+str(EPOCH)+'epoch_' + str(WEIGHTED) + '.csv')
df_test.to_csv('TGCN_Test_Log_'+str(EPOCH)+'epoch_' + str(WEIGHTED) + '.csv')
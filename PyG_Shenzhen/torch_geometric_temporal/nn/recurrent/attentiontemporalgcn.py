...
class A3TGCN2(torch.nn.Module):
    ...
    def _setup_layers(self):
        ...
        self._attention = torch.nn.Parameter(torch.empty(self.time_len, device=device))
        torch.nn.init.uniform_(self._attention)
    ...
    def forward( 
        ...
        H_accum = 0
        probs = torch.nn.functional.softmax(self._attention, dim=0)
        for time_len in range(self.time_len):
            H_accum = H_accum + probs[time_len] * self._base_tgcn( X[:, :, :, time_len], edge_index, edge_weight, H) #([32, 207, 32]
        return H_accum
...
class DCRNN(torch.nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int,batch_size: int, K: int, bias: bool = True):
    ...
    def forward(
        ...
        for batch in range(self.batch_size):
            H = None
            H = self._set_hidden_state(X[batch,:,:], H)
            Z = self._calculate_update_gate(X[batch,:,:], edge_index, edge_weight, H)
            R = self._calculate_reset_gate(X[batch,:,:], edge_index, edge_weight, H)
            H_tilde = self._calculate_candidate_state(X[batch,:,:], edge_index, edge_weight, H, R)
            H = self._calculate_hidden_state(Z, H, H_tilde)
            H = H.unsqueeze(0)
            H_cat.append(H)
        return torch.cat(H_cat,0)

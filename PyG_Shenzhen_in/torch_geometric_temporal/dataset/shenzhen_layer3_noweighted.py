...
class ShenzhenDatasetLoader_layer3_noweighted(object):
    ...
    def _read_web_data(self):
        A = np.array(pd.read_csv(os.path.join(self.raw_data_dir, "in\\adj_layer"+str(LAYER)+".csv"),header=0,index_col=0))
        X = np.expand_dims(np.array(pd.read_csv(os.path.join(self.raw_data_dir, "in\\layer"+str(LAYER)+"_dataset_s.csv"),header=0,index_col=0)),axis=2).transpose((1, 2, 0))
        W = np.array(pd.read_csv(os.path.join(self.raw_data_dir, "in\\adj_layer"+str(LAYER)+"_weighted.csv"),header=0,index_col=0))

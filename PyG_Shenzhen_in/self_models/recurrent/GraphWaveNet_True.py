...
import torch
from torch_geometric_temporal.nn.recurrent import GraphWaveNet
...
class TemporalGNN(torch.nn.Module):
    def __init__(self, adj, hidden_dim, time_len,pre_len):
        ...
        self.tgnn = GraphWaveNet(adj.size(0),1,1,pre_len)
...
for epoch in range(EPOCH):
    ...
    for encoder_inputs, labels in train_loader:
        ...
        encoder_inputs=torch.unsqueeze(encoder_inputs, dim=3)
        y_hat = model(encoder_inputs,static_edge_index,static_edge_weight)  # Get model predictions
        y_hat = torch.squeeze(y_hat.transpose(1,2))
    ...
    with torch.no_grad():
        ...
        for encoder_inputs, labels in test_loader:
            ...
            accuracy_list.append(accuracy(labels.reshape(-1, labels.shape[1]), y_hat.reshape(-1, y_hat.shape[1])))
    print("Epoch {} test Accuracy: {:.6f}".format(epoch, sum(accuracy_list) / len(accuracy_list)))
    print("Epoch {} test RMSE: {:.6f}".format(epoch, sum(total_loss) / len(total_loss)))
...
df_train.to_csv('GraphWaveNet_Train_Log_'+str(EPOCH)+'epoch_' + str(WEIGHTED) + '.csv')
df_test.to_csv('GraphWaveNet_Test_Log_'+str(EPOCH)+'epoch_' + str(WEIGHTED) + '.csv')